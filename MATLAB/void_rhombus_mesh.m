function [NL, EL] = void_rhombus_mesh(d1, d2, p, m, c)
    ...
    for i = 1:p+1
        ...
        coor22(m*(p+1)+i,1) = ((d1-c)/2)+c*sin(beta-alfa)+(i-1)*(c/p);
        coor22(m*(p+1)+i,2) = ((d2+c*cos(beta-alfa))/2);
    ...
    for i = 1:p-1
        ...
        coor33(m*(p-1)+i,1) = ((d1-c)/2)+(i-1)*(c*cos(2*alfa)/p);
        coor33(m*(p-1)+i,2) = ((d2-c)/2)+(i)*(c*1.2*sin(2*alfa)/p);
    ...
    for i = 1:p-1
        ...
        coor44(m*(p-1)+i,1) = ((d1+c)/2)+(i-1)*(c*cos(2*alfa)/(p));
        coor44(m*(p-1)+i,2) = ((d2-c)/2)+(i)*(c*sin(2*alfa)/(p));
